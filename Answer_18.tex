...
When $g(x) < f(x)$, which of the following must also be true?(D)
...
Which of the following equations best represents this model?(B)
...
\]  
Which of the following is the best interpretation of the statement “$m(370)$ is approximately equal to $389$” in this context?(B)
...
What is the percent increase in the number of hours guests used the pool from March to April?(A)
...
For data set A, the table summarizes the distribution of the number of pieces of mail received by a business each day during a period of 11 days.(C)
...
If $\tan(A) = \sqrt{5}$ and $DF = 53$, what is the length of $DE$?(D)

\choicebox{A}{$\dfrac{53\sqrt{3}}{5}$}  
\choicebox{B}{$\dfrac{53\sqrt{3}}{2}$}  
\choicebox{C}{$53\sqrt{5}$}  
\choicebox{D}{$106$}
...
If the surface area of the pyramid is $k\sqrt{3}$ square centimeters, what is the value of $k$?(D)
